/*
    some ext4 hash functions

*/

pub const DIR_SEED: [u32; 4] = [0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476];
...
pub mod hash_versions {
    ...
    // index corresponds to hashtype byte value for ext4 hashdir
...
pub mod dirhash {
    ...
    // create the hash of a directory name used as the key for ext4 hashdir
    pub fn create_dirhash(s: [u32; 4], hash_version: HashVer, filename: &str) -> (u32, u32) {
        // md4 specific
        let mut seed: [u32; 4];
        if s == [0, 0, 0, 0] {
        ...
        match hash_version {
            ...
            _ => {
                panic!("Hash algorithm is not implemented: {:?}: ", hash_version,);
            }
        }
        (seed[1] & !1, seed[2]) // word[1] is 'the most hashed' and is used as the key
    }
    // implementation of str_to_hashbuf from ext4 hashdir utils.
...
pub mod tea {
    ...
    // actually let's just comment it out for now
...
/* ext4 has it's own special implementation of halfmd4
    here's a module that implements it.
*/
pub mod mdfour {
    const K1: u32 = 0;
    ...
    fn round(f: fn(u32, u32, u32) -> u32, a: &mut u32, b: u32, c: u32, d: u32, x: u32, s: u32) {
        let inner = f(b, c, d).wrapping_add(x);
        *a = a.wrapping_add(inner);
        *a = a.rotate_left(s);
    }
    /*
     * Ext4 half-MD4 transform.  Returns only 32 bits of result.
     * Credit to Theodore Ts'o for the implementation in the linux source.
     */

    pub fn half_md4_transform(seed: &mut [u32; 4], data: [u32; 8]) -> u32 {
        let mut a = seed[0];
        let mut b = seed[1];
        let mut c = seed[2];
        let mut d = seed[3];
        ...
        return seed[1];
        // NOTE: ext4 stores 1 and 2 as major and minor hash
        // this function modifies the seed array passed in so both are accessible from there
        // after return.
    }
}
